...
def compute_dL_dz(dL_da,da_dz):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_dz = np.empty([len(dL_da), 1])
    dL_dz = np.matmul(da_dz,dL_da)
...
def compute_dL_dW(dL_dz,dz_dW):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_dW = np.empty([np.shape(dz_dW)[0],np.shape(dz_dW)[1]])
    for i in range(len(dL_dz)):
        dL_dW[i] = np.matmul(np.transpose(dL_dz[i]),dz_dW[i])
...
def predict(Xtest, W, b):
    ...
    for i, x in enumerate(Xtest):
        ...
        z = compute_z(x,W,b)
        label = np.argmax(compute_a(z))
        P[i] = np.transpose(compute_a(z))
        Y[i] = label
        #########################################
    return Y, P 
