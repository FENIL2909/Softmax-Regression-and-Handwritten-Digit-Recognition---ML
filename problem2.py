...
def compute_dz1_dW1(x,h):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz1_dW1 = np.empty([h, len(x)])
    for i in range(h):
        dz1_dW1[i] = np.transpose(x)
...
def compute_dL_da1(dL_dz2,dz2_da1):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_da1 = np.matmul(np.transpose(dz2_da1),dL_dz2)
...
def compute_gradients(dL_da2, da2_dz2, dz2_dW2, dz2_db2, dz2_da1, da1_dz1, dz1_dW1, dz1_db1):
    ...
    # the 2nd layer
    dL_dz2 = np.matmul(da2_dz2, dL_da2)

    dL_da1 = np.matmul(np.transpose(dz2_da1), dL_dz2)

    dL_dW2 = np.empty([np.shape(dz2_dW2)[0], np.shape(dz2_dW2)[1]])

    for i in range(len(dL_dz2)):
        dL_dW2[i] = np.matmul(np.transpose(dL_dz2[i]),dz2_dW2[i])
    ...
    dL_dW1 = np.empty([np.shape(dz1_dW1)[0],np.shape(dz1_dW1)[1]])

    for i in range(len(dL_dz1)):
        dL_dW1[i] = np.matmul(np.transpose(dL_dz1[i]),dz1_dW1[i])
